...
def working_orders() -> list[WorkingOrder]:
    try:
        rows = get_active_client().get_stock_orders()
    except YuantaClientError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    # Working = order accepted (status 20) with unfilled remainder.
    # AfterQty is the current order size and OkQty the filled size, so a
    # fully-filled order has AfterQty == OkQty and must not appear here.
